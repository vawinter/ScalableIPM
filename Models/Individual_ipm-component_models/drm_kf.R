drm_kf <- nimbleCode({
  ...
  #----------------------------------------------------------#
  # # Known-fate model: Cumulative survival ---- 
  #  per age, wmu, and 'cohort' 
  #----------------------------------------------------------#
  ...
  for (t in 1:(true.occasions)) {   
    female.time.effect[t] ~ dnorm(0, sd = 1.5) # tried changing from 0.5 to 1            
  }
  #----------------------------------------------------------#
  ...
  #  female.mean.harv.jv[t] <- (1-female.mean.s.jv[t])*female.seber.recov[1]
    female.mean.harv.ad[t] <- (1-(female.mean.s.ad[t]*female.mean.s.jv[t]))*female.seber.recov[2]
  }
  ...
  for (t in 1:(true.occasions)) {
    for (g in 1:female.n.wmu) {
     # female.h.juv.wmu[t,g] <- (1-female.s.juv.wmu[t,g])*female.seber.recov[1] 
      female.h.ad.wmu[t,g] <- (1-(female.s.ad.wmu[t,g]*female.s.juv.wmu[t,g]))*female.seber.recov[2]
    }#g
  }#t
  ...
  for (i in 1:female.nind){
    ...
    for (t in (female.f[i]+1):female.n.occasions){ # true.occasions + 1 b.c they're dead
      ...
      female.mu1[i,t] <- female.f.kf[i,t-1]*female.s.kf[female.wmu.index[i], female.age[i], 
                                                        tagging.cohort[i]]*female.s[i,t-1]+
