...
qbeta(c(0.025, 0.975), 1, 25)
qbeta(c(0.025, 0.975), 2, 50)

prior_comparison <- data.frame(
  Prior = c("Informed", "Diffuse"),
  Distribution = c("Beta(2, 50)", "Beta(1, 25)"),
  Mean = c(2/(2+50), 1/(1+25)),
  SD = c(sqrt((2*50)/((2+50)^2*(2+50+1))), 
         sqrt((1*25)/((1+25)^2*(1+25+1)))),
  `95% Interval` = c(paste0("[", paste(round(qbeta(c(0.025, 0.975), 2, 50), 3), collapse=", "), "]"),
                     paste0("[", paste(round(qbeta(c(0.025, 0.975), 1, 25), 3), collapse=", "), "]"))
)
...
prior_densities <- data.frame(
  ...
  density = c(dbeta(x, 2, 50), dbeta(x, 1, 25)),
  Prior = rep(c("Beta(2,50)", "Beta(1,25)"), each = 1000)
)


comp <- ggplot(prior_densities, aes(x = harvest_rate, y = density, color = Prior)) +
  geom_line(linewidth = 1.2) +
  labs(x = "Female Harvest Rate", y = "Density", title = "") +
  scale_color_manual(values = c("Beta(2,50)" = "#2C5F7C", "Beta(1,25)" = "#D97E3C")) +
  theme_classic() +
  theme(
...
ggsave("SubmissionMaterial/MinorRevisions/Appx7Fig1.png", plot = comp, device = "png",  bg="transparent",
       width = 10, height = 6, dpi = 700)


beta_2_50_results <- readRDS("Data/Output/R24_abundance_summary.rds")
beta_1_25_results <- readRDS("Data/Output/R24-R2CommentAp7_abundance_summary.rds")
abundance_comparison <- bind_rows(
  beta_2_50_results %>% mutate(Prior = "Beta(2,50)"),
  beta_1_25_results %>% mutate(Prior = "Beta(1,25)")
) %>% 
  filter(sex == "Female") %>% 
...
abun <- ggplot(abundance_comparison, aes(x = year, y = median_value/area_sq_km, 
  ...
  scale_shape_manual(values = c("Beta(2,50)" = 16, "Beta(1,25)" = 17)) + 
  scale_color_manual(values = c("Beta(2,50)" = "#2C5F7C", "Beta(1,25)" = "#D97E3C")) +
...
ggsave("SubmissionMaterial/MinorRevisions/Appx7Fig2.png", plot = abun, width = 15, height = 10, dpi = 700)

# Now, lets looks at the harvest rates
female_harvest_beta_2_50 <- readRDS("Data/Output/R24_harvest_summary.rds")
female_harvest_beta_1_25 <- readRDS("Data/Output/R24-R2CommentAp7_harvest_summary.rds")
# Prepare harvest rate comparison data
harvest_comparison <- bind_rows(
  female_harvest_beta_2_50 %>% mutate(Prior = "Beta(2,50)"),
  female_harvest_beta_1_25 %>% mutate(Prior = "Beta(1,25)")
) %>% 
...
harvest <- harv1/harv2
ggsave("SubmissionMaterial/MinorRevisions/Appx7Fig3.png", plot = harvest, width = 15, height = 10, dpi = 700)
