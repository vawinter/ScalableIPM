# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
############# Operational Integrated Population Model (O_IPM): ################X
#                     #---# PhD Dissertation: O_IPM #---#
...
harvest_plot2 <- drm_harvest_df %>% 
  ...
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5), labels = c("2020", "2021", "2022", "2023", "2024")) +  # Change x-axis labels
  scale_y_continuous(limits = c(0, 0.08)) +  # Set y-axis limits
  annotate("segment", x = Inf, xend = Inf, y = -Inf, yend = Inf, color = "black", linetype = "dashed") +
...
ggsave("Dataviz/O_24_kf_survival_plot.png", plot = kf_survival_plot,  width = 10, height = 6, dpi = 700)
ggsave("Dataviz/O_24_rec_plot.png", plot = recruitment_plot, width = 10, height = 8, dpi = 700)
